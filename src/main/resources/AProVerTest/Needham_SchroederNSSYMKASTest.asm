...
//			        if(symDec(MB,2,1,5,EVE)=true)then
			            if(knowsSymKey(EVE,KAS)=true)then
	   			     par 
			         	messageField(EVE,ALICE,2,MB):=KAS
			         	messageField(EVE,ALICE,4,MB):=KAS
			        	knowsNonce(EVE,messageField(SERVER,EVE,1,MB)):=true
			        	knowsSymKey(EVE,messageField(SERVER,EVE,2,MB)):=true
			        	knowsIdentityCertificate(EVE,messageField(SERVER,EVE,3,MB)):=true
//		            if(symDec(MB,1,4,5,EVE)=true)then
...
default init s0:
	...
	function knowsSymKey($a in Agenti ,$sk in Knowledge)=if(($a=agentA and $sk=KAS) or ($a=agentA and $sk=KAB) or ($a=agentB and $sk=KBS) or ($a=agentB and $sk=KEB) or ($a=agentB and $sk=KAB) or ($a=agentE and $sk=KEA) or ($a=agentE and $sk=KEB) or ($a=agentE and $sk=KES) or ($a=agentE and $sk=KAS) or ($a=agentS and $sk=KAS) or ($a=agentS and $sk=KBS) or ($a=agentS and $sk=KAB)) then true else false endif
