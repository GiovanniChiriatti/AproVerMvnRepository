			...
			if(protocolMessage($a,self)=MB and protocolMessage(self,$b)!=MB and mode=PASSIVE)then
                    ...
                    //                          knowsIdentityCertificate(self,messageField($a,self,1,MB)):=true
                     //                         protocolMessage(self,$b):=MB
			          ...
			          //      endpar
			        if(symDec(MB,1,1,3,self)=true)then
			                par
                                              knowsIdentityCertificate(self,messageField($a,self,1,MB)):=true
                                              knowsOther(self,messageField($a,self,2,MB)):=true
                                              knowsNonce(self,messageField($a,self,3,MB)):=true
                                              protocolMessage(self,$b):=MB
                                              messageField(self,$b,1,MB):=messageField($a,self,1,MB)
			...
			if(protocolMessage($a,self)=MD and protocolMessage(self,$b)!=MD and mode=PASSIVE)then
			        ...
			        if(symDec(MD,1,1,1,self)=true)then
			                par
                                              ...
                                              protocolMessage(self,$b):=MD
                                              messageField(self,$b,1,MD):=messageField($a,self,1,MD)
			                      symEnc(MD,1,1,1):=messageField($b,self,2,MC)
			                endpar
			        else
			                par
                                              protocolMessage(self,$b):=MD
			...
			if(internalStateA(self)=IDLE_MA)then 
			        if(receiver=AG_B)then
			                par
			                       ...
			                       internalStateS(self):=WAITING_MB
			                endpar
			        else
			...
			if(internalStateB(self)=WAITING_MA and protocolMessage($e,self)=MA)then
			     if(receiver=AG_S)then
			                par
			                      ...
			                      messageField(self,$e,2,MB):=messageField($e,self,2,MA)
			                      messageField(self,$e,3,MB):=NB
			                      symEnc(MB,1,1,3):=KBS
			...
			if(internalStateS(self)=WAITING_MB and protocolMessage($e,self)=MB)then
			        if(symDec(MB,1,1,3,self)=true ) then
					      ...
					      knowsOther(self,messageField($e,self,3,MA)):=true
			                      ...
			                      messageField(self,$e,3,MC):=messageField($e,self,2,MB)
			                      messageField(self,$e,4,MC):=messageField($e,self,3,MB)
 			                      symEnc(MC,1,1,4):=KES
			                      messageField(self,$e,1,MC):=messageField($e,self,1,MB)
			                      messageField(self,$e,2,MC):=messageField($e,self,2,MC)
 			                      symEnc(MC,1,1,2):=KES
	...
	rule r_message_MD =
		let ($e=agentE) in
			if(internalStateA(self)=WAITING_MC and protocolMessage($e,self)=MC)then
			     if(receiver=AG_B)then
			        if(symDec(MA,1,1,4,self)=true  && symDec(MB,1,1,2,self)=true)) then
			                par
                            	              knowsIdentityCertificate(self,messageField($e,self,1,MA)):=true
			                      ...
			                      messageField(self,$e,1,MD):=messageField($e,self,1,MC)
			                      messageField(self,$e,2,MD):=messageField($e,self,2,MC)
			                      symEnc(MD,1,1,2):=KBS
			                      messageField(self,$e,1,MD):=messageField($e,self,4,MC)
