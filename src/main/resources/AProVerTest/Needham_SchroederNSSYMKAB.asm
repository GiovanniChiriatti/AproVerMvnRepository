asm Needham_SchroederNSSYMKAB

import CryptoLibraryNSSYMKAB
		  ...
		  par 
			...
			if(protocolMessage(1,$a,self)=MB and protocolMessage(1,self,$b)!=MB and mode=ACTIVE)then
		          par 
			        ...
			        if(symDec(MB,2,1,5,self)=true)then
	   			     par 
			         	messageField(self,$b,2,MB):=KAB
			         	messageField(self,$b,4,MB):=KAB
			        	knowsNonce(self,messageField($a,self,1,MB)):=true
			        	knowsSymKey(self,messageField($a,self,2,MB)):=true
			        	knowsIdentityCertificate(self,messageField($a,self,3,MB)):=true
			            if(symDec(MB,1,4,5,self)=true)then
...
default init s0:
	...
	function knowsSymKey($a in Agent ,$sk in KnowledgeSymKey)=if(($a=agentA and $sk=KAS) or ($a=agentA and $sk=KAB) or ($a=agentB and $sk=KBS) or ($a=agentB and $sk=KEB) or ($a=agentB and $sk=KAB) or ($a=agentE and $sk=KEA) or ($a=agentE and $sk=KEB) or ($a=agentE and $sk=KES) or ($a=agentE and $sk=KAB) or ($a=agentS and $sk=KAS) or ($a=agentS and $sk=KBS) or ($a=agentS and $sk=KAB)) then true else false endif
