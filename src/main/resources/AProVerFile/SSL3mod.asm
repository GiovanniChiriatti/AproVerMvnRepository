	...
	rule r_check_MC =
		let ($e=agentE) in
			if(internalStateB(self)=WAITING_MC and protocolMessage($e,self)=MC)then
			        if(symDec(MC,2,1,2,self)=true and verifySign(MC,1,2,2,self)=true )then
//Aggiunta  if ***********************************************
//					if(verifySign(MC,1,2,2,self)=true)then
